...
🌟 功能特性
交易管理: 创建、查看、更新、删除交易记录

数据持久化: 使用 H2 文件数据库，重启应用数据不丢失

RESTful API: 提供完整的 API 接口，支持数据增删改查操作
...
Maven 3.6 或更高版本(可选，可使用内置的Maven Wrapper)
...
🆘 常见问题
Q: 应用启动后无法访问
A: 检查端口8080是否被其他程序占用，可以更改服务器端口或停止占用端口的程序。

Q: H2控制台无法连接数据库
A: 确保JDBC URL为jdbc:h2:file:./data/transactiondb，用户名和密码正确。

Q: 数据在应用重启后丢失
A: 确保使用的是文件数据库模式(默认配置)，而不是内存数据库模式。

Q: 性能测试时响应缓慢
A: 检查是否开启了SQL日志输出，生产环境应关闭spring.jpa.show-sql。


感谢您使用 Transaction Management System！